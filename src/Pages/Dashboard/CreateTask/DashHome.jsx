...
const Task = ({ task, onTaskMove }) => {
  ...
  const handleDeleteItem = (_id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            axios.delete(`https://scic-job-task-server-three.vercel.app/tasks/${_id}`)
            .then(res => res.json())
            .then(data => {
                console.log(data);
  ...
  return (
    <div ref={drag} className='bg-slate-200 bg-opacity-25 flex justify-between items-center mt-1'>
...
const Column = ({ title, tasks, status, onTaskMove }) => {
  ...
  return (
    <div className='w-[90%] mx-auto rounded-xl lg:w-1/3 h-[300px] bg-slate-100 bg-opacity-25 mt-5' ref={drop}>
...
const DashHome = () => {
  ...
  const { data: allTasks = [], refetch } = useQuery({
    queryKey: ['allTasks'],
    queryFn: async () => {
      const res = await axios.get('https://scic-job-task-server-three.vercel.app/tasks');
      return res.data;
    },
  });
  ...
  const moveTask = async (taskId, fromStatus, toStatus) => {
    try {
      ...
      await axios.patch(`https://scic-job-task-server-three.vercel.app/tasks/${taskId}`, { status: toStatus });
